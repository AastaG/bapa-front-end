...
;(function($){
    $.fn.responsiveGallery = function(option){
		var opts = $.extend({}, $.fn.responsiveGallery.defaults, option), //配置选项
			...
			isAnimating = false,
			touchX = 0;
			console.log($rgWrapper);

		function getTransform3dCSS(tx,ty,ry,zIndex,opacity,visibility,active){
		...
		function getTransform7CSS(){
			...
			if(support3d){
				cssArray = [
					...
					getTransform3dCSS('300%', 	0, 		0,		2,	1,	'visible',1),
		...
		function getTransform5CSS(){
			...
			if(support3d){
				cssArray = [
					getTransform3dCSS('-100%',	-400, 	45,		-1,	0,	'hidden',0),

					getTransform3dCSS('0', 		-300, 	45,		0,	1,	'visible',0),
					getTransform3dCSS('100%', 	-200, 	45,		1,	1,	'visible',0),
					getTransform3dCSS('200%', 	0, 		0,		2,	1,	'visible',1),
					getTransform3dCSS('300%', 	-200, 	-45,	1,	1,	'visible',0),
					getTransform3dCSS('400%', 	-300, 	-45,	0,	1,	'visible',0),

					getTransform3dCSS('500%', 	-400, 	-45,	-1,	0,	'hidden',0)
				];
			} else if(support2d){
				cssArray = [
					getTransform2dCSS('-100%',	0.6,	'100%',		'50%',	0,	'hidden'),
		...
		function getTransform3CSS(){
			...
			if(support3d){
				cssArray = [
					getTransform3dCSS('-100%',	-400, 	45,		0,	0,	'hidden',0),

					getTransform3dCSS('0', 		-300, 	45,		1,	1,	'visible',0),
					getTransform3dCSS('100%', 	0, 		0,		2,	1,	'visible',1),
					getTransform3dCSS('200%', 	-300, 	-45,	1,	1,	'visible',0),

					getTransform3dCSS('300%', 	-400, 	-45,	0,	0,	'hidden',0)
				];
			} else if(support2d){
				cssArray = [
		...
		function getTransform1CSS(){
			...
			if(support3d){
				cssArray = [
					...
					getTransform3dCSS('0%', 	0, 		0,		2,	1,	'visible',1),
		...
		function moveGallery(direction){
			...
			$('.responsiveGallery-wrapper li').find('.conference-detail').hide();
			setTimeout(function(){
				isAnimating = false;
				$('.responsiveGallery-wrapper li').filter(function () { return $(this).css('tab-size') == '1px' }).find('.conference-detail').fadeIn('fast');
				$('.responsiveGallery-wrapper li').removeClass('active');
				$('.responsiveGallery-wrapper li').filter(function () { return $(this).css('tab-size') == '1px' }).addClass('active');
			},animatDuration); //防止连击
		}
